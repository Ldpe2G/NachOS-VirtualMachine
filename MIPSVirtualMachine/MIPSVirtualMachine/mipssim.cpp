...
Machine::Run()
{
    ...
    if (debug->IsEnabled('m')) {
		...
		cout << ", at time: " << stats->totalTicks << "\n";		/* Liang DePeng + */
    }
    //kernel->interrupt->setStatus(UserMode);	/* Liang DePeng - */
	interrupt->setStatus(UserMode);				/* Liang DePeng + */
    for (;;) {
        OneInstruction(instr);
		//kernel->interrupt->OneTick();			/* Liang DePeng - */
		interrupt->OneTick();					/* Liang DePeng + */
		//if (singleStep && (runUntilTime <= kernel->stats->totalTicks))	/* Liang DePeng - */
		if (singleStep && (runUntilTime <= stats->totalTicks))				/* Liang DePeng + */
			Debugger();
    }
}
