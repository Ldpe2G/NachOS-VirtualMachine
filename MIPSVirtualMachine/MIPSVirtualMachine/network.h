...
//#define MaxPacketSize 	(MaxWireSize - sizeof(struct PacketHeader))			/*zhong_tran - */
#define MaxPacketSize 	(MaxWireSize - sizeof(PacketHeader))			/*zhong_tran + */
...
class NetworkInput : public CallBackObj{
  ...
  private:
    //int sock;                   // UNIX socket number for incoming packets	/*zhong_tran - */
	HANDLE hPipe;		/*zhong_tran + */
	LPOVERLAPPED lpOverlapped;		/*zhong_tran + */
    //char sockName[32];          // File name corresponding to UNIX socket		/*zhong_tran - */
	char pipeName[32];		/*zhong_tran + */
	//LPCWSTR pipeName;
...
class NetworkOutput : public CallBackObj {
  ...
  private:
    //int sock;                   // UNIX socket number for outgoing packets	/*zhong_tran - */
