...
class PostOfficeInput : public CallBackObj {
  ...
  private:
    ...
    FSemaphore *messageAvailable;// V'ed when message has arrived from network
};
...
class PostOfficeOutput : public CallBackObj {
  public:
    ...
    void Send(PacketHeader pktHdr, MailHeader mailHdr, char *data, int hostname);
  ...
  private:
    ...
    FSemaphore *messageSent;	// V'ed when next message can be sent to network
    FLock *sendLock;		// Only one outgoing message at a time
};
