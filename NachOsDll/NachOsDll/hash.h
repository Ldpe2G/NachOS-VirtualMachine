...
class MyHashTable {
  public:
    MyHashTable(Key* (*get)(Item* x), unsigned int (*hFunc)(Key* x), bool (* keyEqual)(Key* k1, Key* k2));	
    				// initialize a hash table
    ~MyHashTable();		// deallocate a hash table

    void Insert(Item* item);	// Put item into hash table
    Item* Remove(Key* key);		// Remove item from hash table.

    bool Find(Key* key, Item** itemPtr) const; 
    				// Find an item from its key
    bool IsInTable(Key* key) { Item* dummy; return Find(key, &dummy); } 	
				// Is the item in the table?

    bool IsEmpty() { return numItems == 0; }	
				// does the table have anything in it

    void Apply(void (*f)(Item*)) const;
    				// apply function to all elements in table

    void SanityCheck() const;// is this still a legal hash table?
    void SelfTest(Item** p, int numItems);	
...
private:
    ...
    Key* (*getKey)(Item* x);		// get Key from value
    unsigned (*hash)(Key* x);	// the hash function
	bool (* kkeyEqual)(Key* k1, Key* k2);
    ...
    bool FindInBucket(int bucket, Key* key, Item** itemPtr) const; 
    				// find item in bucket
...
class HashIterator {
  public:
    HashIterator(MyHashTable *table); // initialize an iterator
    ...
    Item* Item() { ASSERT(!IsDone()); return bucketIter->GetItem(); }; 
				// return current item in table
    void Next(); 		// update iterator to point to next

private:   
    MyHashTable *table;	// the hash table we're stepping through
    int bucket;			// current bucket we are in
    ListIterator *bucketIter; // where we are in the bucket
};
/* Liang DePeng +
 ****/
