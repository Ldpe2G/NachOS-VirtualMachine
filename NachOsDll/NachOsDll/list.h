...
class ListElement {
  public:
    ListElement(Item* itm); 	// initialize a list element
    ListElement *next;	     	// next element on list, NULL if this is last
...
//template <class T> class ListIterator; /* Liang Depeng - */
class ListIterator; /* Liang Depeng + */
...
class __declspec(dllexport) List {
  public:
	List(){};			// initialize the list
	virtual ~List() = 0{};		// de-allocate the list

    virtual void Prepend(Item* item) = 0;// Put item at the beginning of the list
    virtual void Append(Item* item) = 0; // Put item at the end of the list

    virtual Item* Front() = 0; //{ return first->item; }
    				// Return first item on list
				// without removing it
    virtual Item* RemoveFront() = 0; 		// Take item off the front of the list
    virtual void Remove(Item* item) = 0; 	// Remove specific item from list

    virtual bool IsInList(Item* item) const = 0;// is the item in the list?

    virtual unsigned int NumInList() = 0 ;//{ return numInList;};
    				// how many items in the list?
    virtual bool IsEmpty() = 0;//{ return (numInList == 0); };
    				// is the list empty? 

    virtual void Apply(void (*f)(Item*)) const = 0; 
    				// apply function to all elements in list

	virtual void Insert(Item* item) = 0; 	// insert an item onto the list 

    virtual void SanityCheck() const = 0;	
				// has this list been corrupted?
    virtual void SelfTest(Item** p, int numEntries) = 0;
				// verify module is working
...
class ListIterator {
  public:
    ListIterator(List *list) { current = list->first; } 
				// initialize an iterator

    bool IsDone() { return current == NULL; };
				// return TRUE if we are at the end of the list

    Item* GetItem() { ASSERT(!IsDone()); return current->item; };
				// return current element on list
  ...
  private:
    ListElement *current;	// where we are in the list
};
/* Liang DePeng +
 ****/
...
//宏定义函数指针
typedef List* (*GetSortedList)(int (*comp)(Item* x, Item* y));  
typedef void (*DeleteSortedList)(List*);

extern "C" {
	__declspec(dllexport) List* getSortedList(int (*comp)(Item* x, Item* y));
	__declspec(dllexport) void deleteSortedList(List*);
}

extern GetSortedList getList;
extern DeleteSortedList deleteList;	
/* Liang DePeng +
 ****/


//#include "list.cpp"		// templates are really like macros
