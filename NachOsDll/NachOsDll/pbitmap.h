...
class PersistentBitmap : public Bitmap {
  public:
    PersistentBitmap(OpenNaFile *file,int numItems); //initialize bitmap from disk 
    PersistentBitmap(int numItems); // or don't...

    ~PersistentBitmap(); 			// deallocate bitmap

    void FetchFrom(OpenNaFile *file);     // read bitmap from the disk
    void WriteBack(OpenNaFile *file); 	// write bitmap contents to disk 
};
