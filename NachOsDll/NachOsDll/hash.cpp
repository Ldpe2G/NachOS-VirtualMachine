...
//template <class Key, class T>		/* Liang DePeng - */
//HashTable<Key,T>::HashTable(Key (*get)(T x), unsigned (*hFunc)(Key x))	/* Liang DePeng - */
MyHashTable::MyHashTable(Key* (*get)(Item* x), unsigned int (*hFunc)(Key* x), bool (* keyEqual)(Key* k1, Key* k2)){		/* Liang DePeng + */
    numItems = 0;
...
//----------------------------------------------------------------------
static int MyfakeCompare(Item* x, Item* y){
	return 0;
}
//template <class Key, class T>					/* Liang DePeng - */
//void HashTable<Key,T>::InitBuckets(int sz){	/* Liang DePeng - */
void MyHashTable::InitBuckets(int sz){			/* Liang DePeng + */
    numBuckets = sz;
    buckets = new Bucket[numBuckets];
    for (int i = 0; i < sz; i++) {
		buckets[i] = getSortedList(MyfakeCompare);	/* Liang DePeng + */
    	//buckets[i] = new List<T>;				/* Liang DePeng - */
    }
}
...
//template <class Key, class T>		/* Liang DePeng - */
//HashTable<Key,T>::~HashTable(){	/* Liang DePeng - */
MyHashTable::~MyHashTable(){			/* Liang DePeng + */
...
//template <class Key, class T>			/* Liang DePeng - */
//void HashTable<Key,T>::DeleteBuckets(List<T> **table, int sz){	/* Liang DePeng - */
void MyHashTable::DeleteBuckets(List **table, int sz){				/* Liang DePeng + */
    for (int i = 0; i < sz; i++) {
    	//delete table[i];			/* Liang DePeng - */
		deleteSortedList(table[i]);	/* Liang DePeng + */
    }
    delete [] table;
}
...
//template <class Key, class T>				/* Liang DePeng - */
//int HashTable<Key, T>::HashValue(Key key) const {	/* Liang DePeng - */
int MyHashTable::HashValue(Key* key) const{	/* Liang DePeng - */
...
//template <class Key, class T>		/* Liang DePeng - */
//void HashTable<Key,T>::Insert(T item){	/* Liang DePeng - */
void MyHashTable::Insert(Item* item){		/* Liang DePeng + */
    Key* key = (* getKey)(item);
...
//template <class Key, class T>		/* Liang DePeng - */
//void HashTable<Key,T>::ReHash(){	/* Liang DePeng - */
void MyHashTable::ReHash(){			/* Liang DePeng + */
...
//template <class Key, class T>		/* Liang DePeng - */
//bool HashTable<Key,T>::FindInBucket(int bucket, Key key, T *itemPtr) const{	/* Liang DePeng - */
bool MyHashTable::FindInBucket(int bucket, Key* key, Item** itemPtr) const{		/* Liang DePeng + */

    //ListIterator<T> iterator(buckets[bucket]);	/* Liang DePeng - */
	ListIterator iterator(buckets[bucket]);			/* Liang DePeng + */

    for (; !iterator.IsDone(); iterator.Next()) {
		//if (key == (* getKey)(iterator.GetItem())) { // found!
		if((* kkeyEqual)(key, (* getKey)(iterator.GetItem()))){
			*itemPtr = iterator.GetItem();
			return TRUE;
		}
    }
    *itemPtr = NULL;
...
//template <class Key, class T>			/* Liang DePeng - */
//bool HashTable<Key,T>::Find(Key key, T *itemPtr) const{	/* Liang DePeng - */
bool MyHashTable::Find(Key* key, Item** itemPtr) const{		/* Liang DePeng + */
...
//template <class Key, class T>			/* Liang DePeng - */
//T HashTable<Key,T>::Remove(Key key){	/* Liang DePeng - */
Item* MyHashTable::Remove(Key* key){		/* Liang DePeng + */
    int bucket = HashValue(key);
    Item* item;
...
//template <class Key,class T>		/* Liang DePeng - */
//void HashTable<Key,T>::Apply(void (*func)(T)) const{	/* Liang DePeng - */
void MyHashTable::Apply(void (*func)(Item*)) const{		/* Liang DePeng + */
...
//template <class Key,class T>			/* Liang DePeng - */
//int HashTable<Key,T>::FindNextFullBucket(int bucket) const{	/* Liang DePeng - */
int MyHashTable::FindNextFullBucket(int bucket) const{			/* Liang DePeng + */
...
//template <class Key, class T>					/* Liang DePeng - */
//void HashTable<Key,T>::SanityCheck() const{	/* Liang DePeng - */
void MyHashTable::SanityCheck() const{			/* Liang DePeng + */
    int numFound = 0;
	//ListIterator<T> *iterator;	/* Liang DePeng - */
	ListIterator* iterator;			/* Liang DePeng + */

    for (int i = 0; i < numBuckets; i++) {
		buckets[i]->SanityCheck();
		numFound += buckets[i]->NumInList();
		//iterator = new ListIterator<T>(buckets[i]);	/* Liang DePeng - */
		iterator = new ListIterator(buckets[i]);		/* Liang DePeng + */
        for (; !iterator->IsDone(); iterator->Next()) {
			ASSERT(i == HashValue((* getKey)(iterator->GetItem())));
        }
        delete iterator;
    }
...
//template <class Key, class T>			/* Liang DePeng - */
//void HashTable<Key,T>::SelfTest(T *p, int numEntries){	/* Liang DePeng - */
void MyHashTable::SelfTest(Item** p, int numEntries){			/* Liang DePeng + */
    int i;
    //HashIterator<Key, T> *iterator = new HashIterator<Key,T>(this);	/* Liang DePeng - */
	HashIterator* iterator = new HashIterator(this);					/* Liang DePeng + */
...
//template <class Key, class T>			/* Liang DePeng - */
//HashIterator<Key,T>::HashIterator(HashTable<Key,T> *tbl) {	/* Liang DePeng - */
HashIterator::HashIterator(MyHashTable *tbl) {	/* Liang DePeng + */
    table = tbl;
    bucket = table->FindNextFullBucket(0);
    bucketIter = NULL;
    if (bucket < table->numBuckets) {
		bucketIter = new ListIterator(table->buckets[bucket]);		/* Liang DePeng + */
		//bucketIter = new ListIterator<T>(table->buckets[bucket]);	/* Liang DePeng - */
    }
}
...
//template <class Key,class T>			/* Liang DePeng - */
//void HashIterator<Key,T>::Next() {	/* Liang DePeng - */
void HashIterator::Next(){				/* Liang DePeng + */
    bucketIter->Next();
