...
/*class OpenNaFile {
  public:
    OpenNaFile(int sector, int sectorSize);		// Open a file whose header is located
								// at "sector" on the disk
    ~OpenNaFile();			// Close the file
...
/**** add
 * Liang DePeng + */
class  __declspec(dllexport) OpenNaFile {
  public:
    OpenNaFile(int sector, int sectorSize);		// Open a file whose header is located
								// at "sector" on the disk
	virtual ~OpenNaFile(){
    ...
    virtual void Seek(int position) = 0; 		// Set the position from which to 
					// start reading/writing -- UNIX lseek
...
typedef OpenNaFile* (*GetOpenNaFile)(int sector, int sectorSize);  
typedef void (*DeleteOpenNaFile)(OpenNaFile*);

extern "C" {
	__declspec(dllexport) OpenNaFile* GetOpenNaFileF(int sector, int sectorSize);
	__declspec(dllexport) void DeleteOpenNaFileF(OpenNaFile*);
}

extern GetOpenNaFile getOpenNaFile;
extern DeleteOpenNaFile deleteOpenNaFile;	
